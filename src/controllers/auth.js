...
module.exports = {
  ...
  adminController: async (req, res) => {
    let {id, role_id, login_as, adminId} = req.user
    console.log(adminId)
    adminId && (adminId = id)
    console.log(adminId)
    if(role_id === 1|2){
  ...
  signupController: async (req, res) => {
    ...
    if (role_id === 'customer') {role_id = 4}
    else if (role_id === 'seller') {role_id = 3}
    else { return responseStandard(res, 'Forbidden input!', {}, 500, false) }
    try {
      let form = await userValidate(req.body, role_id)
      console.log(form)
      let [userCredentials, userDetails] = form
      ...
      if (result.length){
        ...
        return responseStandard(res, 'item has been created', {data:{...userCredentials, ...userDetails}}, 201)
      } else {
